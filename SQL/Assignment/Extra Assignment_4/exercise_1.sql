...
delimiter $$
create procedure delete_project (out num int)
begin
	SELECT 
		count(*) into num
	FROM
		projects
	WHERE
		ProjectCompletedOn < (SELECT SUBDATE(NOW(), INTERVAL 3 MONTH));
...
DROP PROCEDURE NV_lam_viec_ko_giao;
DELIMITER $$
